...
mjd = Time(56080.071231244146,format='mjd',scale='tt').mjd
...
while time<finish:
    ...
    time+=d_doppler/(3600*24)
    time_jd+=d_doppler/(3600*24)
    mjd = Time(time,format='mjd', scale='tt').mjd
    print "Delaying {0} percent".format((time/finish)*100)
    arrival.append(time)
...
with open("GenFromStapper.dat","w") as data:
    data.write("GENERATED TIME\t\t\t ORIGINAL TIME\t\t\t SECONDS OFFSET\n")
    for i in range(len(time_arrival)):
        data.write("{0}\t{1}\t{2}\n".format(time_arrival[i],original_time[i],diff[i]))

"""real_time=real_time[:104]
...
for i in range(len(real_time)):
    ...
    point=point[i]-int(point[i])
    phase_model.append(point)
plt.plot(real_time,phase_model)
plt.xlabel("Time (mjd)")
plt.ylabel("Phase")
plt.savefig("StapperData.png")"""
