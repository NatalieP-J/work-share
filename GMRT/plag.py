...
def im(i, j, nt_l=False, nt_u=False):
    if not nt_l:
        M = transpose(abs(getlag(gmap(i,j,nchan))))
        imshow(M, extent=[0,2.5,0,1.5])
        [vlines(2.5/16.*(1+i),0,1.5,color='green') for i in range(16)]
        ax = pylab.gca()
        ...
        M = transpose(abs(getlag2(gmap(i,j,nchan),nt_l,nt_u)))
        imshow(M, extent=[0,2.5,0,1.5])
        [vlines(2.5/16.*(1+i),0,1.5,color='green') for i in range(16)]
        ax = pylab.gca()
        ax.yaxis.set_visible(False)
