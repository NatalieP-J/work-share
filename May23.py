...
#Required arguments
parser.add_argument('--i', type=float, required=True, help=''' The inclination of the orbit in arcseconds''')
parser.add_argument('--l', type=float, required=True, help=''' The longitude of the ascending node in arcseconds''')
...
if time==None and n==None:
    print 'Please choose a time or a number of measurements and orbits.'
if time==None and orb==None:
    time=np.linspace(0,2*T*np.pi,n)
if time==None:
    time=np.linspace(0,orb*2*T*np.pi,n*orb)
if ra_pm==None:
    ra_pm=0
if dec_pm==None:
    dec_pm=0
...
points=np.array(apparent_orbit(time,t_0,maj,i,T,l,ra_pm,dec_pm))
ra=points[:,0]
dec=points[:,1]
try: 
    len(time)
