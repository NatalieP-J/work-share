...
class Observatory(dict):
    ...
    def ha2za(self, ha, d):
        ...
        haversin(theta) = haversin(dec2-dec1) +cos(dec2)*cos(dec1)*haversin(ra2-ra1)
...
def print_phases(psr, ist_date1='2013-06-16', ist_date2='2013-07-02'):
    ...
    for mjd in np.arange(mjd1, mjd2+1.e-5):
        ...
        print time0.iso[:10], ':', ' '.join(['{:02d}'.format(int(round(phase*100.)) %100) for phase in phaselist])
        
gmrt = Observatory(74*u.deg+02*u.arcmin+59.07*u.arcsec,
                   19*u.deg+05*u.arcmin+47.46*u.arcsec, 'GMRT')
gmrt.zamax = 73.*u.deg # from www.ncra.tifr.res.in/ncra/gmrt/gtac/GMRT_status_doc_Dec_2012.pdf‎
...
b1957.set_ephemeris(tasc=Time(51260.194925280940172, format='mjd', scale='tdb'),
                    porb=0.38196748020990333082 * u.day)

if __name__ == '__main__':
    print 'Source Obs.             HA  LocSidTime UnivSidTime'
    for src in j1012, b1957:
        gmststart = -100.
        gmststop = +100.
        for obs in gmrt, lofar, aro:
            ...
            if hamax < 12.:
                ...
                if gmstmin > gmststart: gmststart = gmstmin
                lstmax = src.ra.hours + hamax
                gmstmax = -obs['l'].to(u.degree).value/15. + lstmax
                if gmstmax < gmststop: gmststop = gmstmax
                print '{:6s} {:6s}(za<{:2d}) ±{:4.1f}: {:04.1f}-{:04.1f} = {:04.1f}-{:04.1f}'.format(
                    src.name, obs['name'], 
                    int(round(obs.zamax.to(u.deg).value)), hamax, 
                    np.mod(lstmin,24.), np.mod(lstmax,24.),
                    np.mod(gmstmin,24.), np.mod(gmstmax,24.))
            else:
                print '{:6s} {:6s}(za<{:2d}) ±12.0: ++++-++++ = ++++-++++'.format(
                    src.name, obs['name'], 
                    int(round(obs.zamax.to(u.deg).value)))
                
        if gmststart >= gmststop:
            print '{:6s} all      ---:             ---- ----\n'.format(src.name,
                np.mod(gmststart,24.), np.mod(gmststop,24.))
        else:
            print '{:6s} all            {:4.1f}:             {:04.1f}-{:04.1f}'.format(src.name,
                gmststop-gmststart, np.mod(gmststart,24.), np.mod(gmststop,24.))

        # get corresponding orbital phases for a range of dates
            ist_date1='2013-06-16 12:00:00'
            ist_date2='2013-07-02 12:00:00'
            ...
            for mjd in np.arange(mjd1, mjd2+1.e-5):
                ...
                print '{}-{}: {:4.2f}-{:4.2f}'.format(
                    ist_start.iso, ist_stop.iso[11:], ph_start, ph_stop)
