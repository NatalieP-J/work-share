...
#define the speed of light (from astropy)
c=299792458.0
...
for i in range(len(glomp)):
    #in addition to removing parallactic angle, convert delay to microseconds
    point=((float(glomp[i][:-9])/c)*10**6
    delay.append(point)

#write the results to file - one column antenna number, the other delays
WriteFileCols(antenna,delay,"micro_delay_{0}".format(fname))
